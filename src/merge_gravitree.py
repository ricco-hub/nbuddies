...
class Node():
    def __init__(self, bounds : list[list[float : 2] : 3], enclosed_blackholes : list[BlackHole] = np.empty(0)):
        ...
        assert self._compute_box_radius(enclosed_blackholes) != 0, "Box radius is 0 (somehow)"
        self.radius = self._compute_box_radius(enclosed_blackholes)
        self.center = self._compute_node_center()
    ...
    def has_children(self):
        """
        determines if this node has children based on knowledge 
        nodes with less than 2 bhs will not be given children when the tree is initialized
    ...
    def _compute_box_radius(self, BHs: list[BlackHole]) -> float:
        # --- Case 1: No black holes in this node ---
        if len(BHs) == 0:
            # fallback: use half the node bounds as radius
            # prevents np.max([]) crash
            return max(
                (self.bounds[0,1] - self.bounds[0,0]) / 2,
                (self.bounds[1,1] - self.bounds[1,0]) / 2,
                (self.bounds[2,1] - self.bounds[2,0]) / 2
            )

        # --- Case 2: Exactly one black hole ---
        if len(BHs) == 1:
            # return a very small non-zero radius (avoids assertion)
            return 1e-12

        # --- Case 3: Normal case (2+ BHs) ---
        center = [
            np.mean([bh.position[0] for bh in BHs]),
            np.mean([bh.position[1] for bh in BHs]),
            np.mean([bh.position[2] for bh in BHs]),
        ]

        box_radii = np.array([
            np.max([np.abs(bh.position[0] - center[0]) for bh in BHs]),
            np.max([np.abs(bh.position[1] - center[1]) for bh in BHs]),
            np.max([np.abs(bh.position[2] - center[2]) for bh in BHs])
        ])

        # enforce cubic bounding box
        box_radius = np.max(box_radii)

        # ensure non-zero radius
        return max(box_radius, 1e-12)

    
    def _compute_node_center(self):
        """
        Compute geometric center of node
        """

        center = [
            (self.bounds[0,0] + self.bounds[0,1])/2,
            (self.bounds[1,0] + self.bounds[1,1])/2,
            (self.bounds[2,0] + self.bounds[2,1])/2
        ]
...
# BH_1 = BlackHole(1.0, [0, 0.1, 0.2], [0.2, 0.4, 0.6])
# BH_2 = BlackHole(2.0, [0.2, 0.4, 0.6], [0, 0.1, 0.2])
# test_node = Node([[0, 1], [0, 1], [0, 1]], [BH_1, BH_2])
# print(test_node.node_radius)